...
import com.nicadevelop.nicavpn.databinding.DialogConfigFilterBinding
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun startListenBroadcast() {
        ...
        getApplication<MyApplication>().registerReceiver(mMsgReceiver, IntentFilter(AppConfig.BROADCAST_ACTION_ACTIVITY))
        MessageUtil.sendMsg2Service(getApplication(), AppConfig.MSG_REGISTER_CLIENT, "")
    }

    override fun onCleared() {
        getApplication<MyApplication>().unregisterReceiver(mMsgReceiver)
    ...
    fun testAllTcping() {
        ...
        getApplication<MyApplication>().toast(R.string.connection_test_testing)
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_START_SUCCESS -> {
                    // TastyToast.makeText(MyApplication.appContext, getApplication<MyApplication>().getString(R.string.toast_services_success), Toast.LENGTH_LONG, TastyToast.SUCCESS).show()
                    // getApplication<NdCheckMateApplication>().toast(R.string.toast_services_success)
                    val i = Intent()
                    i.action = "com.nicadevelop.nicavpn.OSADS"
                    i.`package` = AppConfig.ANG_PACKAGE
                    ctx!!.sendBroadcast(i)

                    isRunning.value = true
                }
                AppConfig.MSG_STATE_START_FAILURE -> {
                    getApplication<MyApplication>().toast(R.string.toast_services_failure)
                    isRunning.value = false
                }
